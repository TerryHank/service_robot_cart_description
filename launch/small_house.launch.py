#!/usr/bin/env python3
"""AWS small house scene + robot + SLAM + fake laser"""
import subprocess
...
def generate_launch_description():
    ...
    spawn = TimerAction(period=8.0, actions=[
        Node(package="ros_gz_sim", executable="create",
             arguments=["-name", "service_robot_cart", "-topic", "robot_description", "-x", "-2.0", "-y", "0.0", "-z", "0.05"],
             output="screen")])
    ...
    slam_lifecycle = Node(package="nav2_lifecycle_manager", executable="lifecycle_manager",
                          ...
                          parameters=[{"use_sim_time": True, "autostart": True, "node_names": ["slam_toolbox"]}])
    ...
    return LaunchDescription([
        ...
        gz_sim, rsp, spawn, bridge, jsb, diff, twist_bridge, fake_scan, slam, slam_lifecycle,
    ])
